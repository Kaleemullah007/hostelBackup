...
function search_by_date_range()
{
	...
	$('button[name=btn-date-range]').attr('disabled','disabled');
	      $.ajax({
	      url: "./get-pending-staff-sallary-by-daterange",
	      method:"post",
...
function search_by_gender()
{
  gender1 = $('select[name=gender]').val();
	...
	$('button[name=btn-gender]').attr('disabled','disabled');
	      $.ajax({
	      url: "./get-pending-staff-sallary-by-gender",
	      method:"post",
	      data:{gender:gender1},
	      success:function(data){
	         setTimeout(function(){
...
function search_by_month_year()
{
	...
	$('button[name=btn-month-year]').attr('disabled','disabled');
	      $.ajax({
	      url: "./get-pending-staff-sallary-by-monthyear",
	      method:"post",
...
function search_by_designation()
{
	...
	$('button[name=btn-designation]').attr('disabled','disabled');
	      $.ajax({
	      url: "./get-pending-staff-sallary-by-designation",
	      method:"post",
	      data:{desig:desig1},
	      success:function(data){
	         setTimeout(function(){
...
function forward_to_accounts(elems)
{
	...
	$('.btn').attr('disabled','disabled');
	      $.ajax({
	      url: "./admin-forward-to-accounts-list",
	      method:"post",
	      data:{pending:elems},
	      success:function(data){
	         setTimeout(function(){
	         if(data[0].error == false){
	         $('#message').html('<div class="alert alert-success"><span class="fa fa-smile-o">&nbsp;</span>List Has Been Sent To Accounts</div>');}
	     	 else{
	     	 $('#message').html('<div class="alert alert-danger"><span class="fa fa-frown-o">&nbsp;</span>Sorry Complete List Not Sent To Accounts Try Again</div>');}
...
function forward_to_accounts_single(elems)
{
	var filter = $('select[name=search-filter]').val();
	$('#inner-content').html('');
	$('#progress-content').css('display','block');
	...
	$('.btn').attr('disabled','disabled');
	      $.ajax({
	      url: "./admin-forward-to-accounts-single",
	      method:"post",
	      data:{pending:elems},
	      success:function(data){
	         setTimeout(function(){
	         if(data.error == false){
	         $('#message').html('<div class="alert alert-success"><span class="fa fa-smile-o">&nbsp;</span>Request Has Been Sent To Accounts</div>');}
	     	 else{
	     	 $('#message').html('<div class="alert alert-danger"><span class="fa fa-frown-o">&nbsp;</span>Sorry Request Not Sent To Accounts Try Again</div>');}
	         $('#message').css('display','block');
	         $('select[name=search-filter]').removeAttr('disabled','disabled');  
	         $('.btn').removeAttr('disabled','disabled');
	         $('#progress-content').css('display','none');
