...
foreach ($data as $value) { ?>
	<tr>
		...
		<td class="no-print">
		<?php if($value->sallary_hr_status == 0){ ?>
		<button type="button" class="btn btn-warning" id="<?php echo $value->sallary_id; ?>" onclick="forward_to_admin_single(this.id)"><i class="fa fa-mail-forward"></i>Pending</button>
		<?php $pending_ids[] = $value->sallary_id; } 
		elseif($value->sallary_hr_status == 1){ ?>
        <button type="button" class="btn btn-success"><i class="fa fa-mail-forward">&nbsp;&nbsp;</i>Sent</button>
        <?php  }?>
		</td>
	</tr>
<?php } } else{ ?>
...
<tr role="row" class="no-print">
 <td colspan="8">
   ...
   <button type="button" class="btn btn-success forward" id='<?php echo json_encode($pending_ids); ?>' onclick="forward_to_admin(this.id)"><i class="fa fa-mail-forward"></i>Forward To Admin</button>
   <?php } ?>
 </td>
</tr>
 <!-- /.forward-button-row -->
